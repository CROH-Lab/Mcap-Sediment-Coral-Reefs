...
```{r}
source("/Users/alexgood/Desktop/Coding Files/Source Code/diagPlots.R")
source("/Users/alexgood/Desktop/Coding Files/Source Code/multcompUtilities.R")
source("/Users/alexgood/Desktop/Coding Files/Source Code/ggplotUtilities.R")
source("/Users/alexgood/Desktop/Coding Files/Source Code/correctedLMEAnova.R")
source("/Users/alexgood/Desktop/Coding Files/Source Code/profilePlot.R")
```
...
## Gnet raw data
```{r}
ylab = expression("Net Calcification" ~ "(μmol CaCO"[3]~"g"^{-1}~h^{-1}*")")
...
### OLD MODELS
# PCA - FAMD because continuous and factor values 
```{r}
Gnet_subset <- gnet[, c("Location", "Treatment", "Timepoint", "Gnet2", "Turbidity", "Colony")]

# Load the package
library(FactoMineR)

# Perform FAMD on the mixed data (both continuous and factors)
famd_result <- FAMD(Gnet_subset)
...
adding a random effect, individual coral, so it will account for the repeated measures 
timepoint can be a factor 
make sure theres no correlation
run the GLMM 
analyses done on the residuals - qqplot
vignette for lme4 or nlme 
```{r}
model <- lmer(Gnet2 ~ Turbidity + Treatment + Location + Colony + Timepoint + (1 | Coral), data = gnet)
summary(model)
